...
async function predict() {
  ...
  // let tensorImg = tf.browser.fromPixels(imagePreview).resizeNearestNeighbor([28, 28]).toFloat().expandDims();
  // prediction = await model.predict(tensorImg).data();
  // console.log(prediction)

  var tfImg = tf.fromPixels(imagePreview, 1);
  ...
  for (index = 0; index < predictedValues.length; index++) {
      if (predictedValues[index] > 0.5) {
          ...
          predResult.innerHTML = '<br/>Predicted Number: ' + index;
      }
  }
  if (!isThereAnyPrediction) {
      predResult.innerHTML = '<br>Unable to Predict';
  }
